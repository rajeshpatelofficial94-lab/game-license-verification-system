<template>
  <div class="bg-white shadow-lg rounded-xl p-6 max-w-lg w-full mx-auto mt-10 border border-gray-100">
    <h2 class="text-2xl font-extrabold mb-6 text-blue-700 text-center tracking-tight">GDPR compliance for regions</h2>
    ...
    <div v-if="regions.length" class="mt-6">
      <strong class="block mb-2 text-gray-700">Available Regions:</strong>
      <div class="flex flex-wrap gap-2">
        <span
          v-for="region in regions"
          :key="region"
          class="bg-blue-100 text-blue-700 px-3 py-1 rounded-full font-medium shadow-sm border border-blue-200"
        >
          {{ region }}
        </span>
      </div>
    ...
    <div v-else-if="!error && gameId" class="mt-6 text-gray-500 text-center font-semibold">
      No regions found for this game.
    </div>
...
export default defineComponent({
  setup() {
    ...
    const getRegions = async () => {
      ...
      try {
        const response = await axios.get('/api/license/regions', {
          params: { game_id: trimmedGameId },
        });
        regions.value = response.data.available_regions;
      } catch (e: any) {
        error.value = e.response?.data?.message || 'An error occurred';
      }
    };

    return { gameId, regions, error, getRegions };
  },
});
</script>
